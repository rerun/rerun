...
If a command option is not set by the user, the
...
new default value and upate the `options.sh` script.
...
You might be interested in what happens behind the scenes, 
when `add-options` generates the `options.sh` script.
Below you can see the `options.sh` script defines a while loop 
...
JUMPS variable being set to a non empty string.
...
create a script called `Linux.sh`.
