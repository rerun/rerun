...
mkdir -p $RERUN_MODULES/$MODULE/etc/commands/$COMMAND
(
    cat <<EOF
...
EOF
) > $RERUN_MODULES/$MODULE/etc/commands/$COMMAND/options.sh || error

# Done
echo "Wrote option: $RERUN_MODULES/$MODULE/etc/commands/$COMMAND/$NAME.option"
